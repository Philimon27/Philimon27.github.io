...
describe("Partial application for login", function () {
    ...
    it("tests rockstar", function () {
        assert.strictEqual(askPassword2(() => user2.loginOk.call(user2), () => user2.loginFail.call(user2), "rockstar"), "John logged in");
    });

    it("tests wrong password", function () {
        assert.strictEqual(askPassword2(() => user2.loginOk.apply(user2), () => user2.loginFail.apply(user2), "1234"), "John failed to log in");
    });
