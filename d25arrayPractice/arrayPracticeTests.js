"use strict";
/* global assert doubleNums  doubleAges filterEven filterOver10 findEvenNum findEvenAge reduceSum  reduceMax reduceAverage reduceMaxOfAges chainEvenAverage chainOddAverage*/
...
describe("reduce", function () {
    ...
    it("sums the number in the array", function () {
        assert.deepEqual(reduceSum(numArray), 14);
        assert.deepEqual(numArray, [7, -2, 3, 4, 2]);  //test for pure function
    });

    it("gets the max", function () {
        assert.deepEqual(reduceMax(numArray),7)
        assert.deepEqual(numArray, [7, -2, 3, 4, 2]);  //test for pure function
    });

    it("gets the average", function () {
        assert.deepEqual(reduceAverage(numArray),2.8)
        assert.deepEqual(numArray, [7, -2, 3, 4, 2]);  //test for pure function
    });
    it("max age", function () {
        assert.deepEqual(reduceMaxOfAges(peopleArray),80);
        assert.deepEqual(peopleArray,
            [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
    it("max age", function () {
        assert.deepEqual(chainEvenAverage(peopleArray),43);
        assert.deepEqual(peopleArray,
            [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
    it("max age", function () {
        assert.deepEqual(chainOddAverage(peopleArray),14);
        assert.deepEqual(peopleArray,
            [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
